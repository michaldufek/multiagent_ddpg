...
class VacuumCleanerEnv:
    ...
    def __init__(self):
        ...
        self.agent_size = 3 # size of the agent (agent occupies n x n cells)
    ...
    def _calculate_rewards(self, actions):
        ...
        for agent_id in self.agents:
            reward = step_penalty # loss paid at each step

            x, y = self.agent_positions[agent_id]

            if self.grid[x, y] == 0:  # Reward for cleaning a dirty cell
                reward += cleaning_reward
            if self._is_obstacle(x, y):
                reward += obstacle_penalty
            if actions[agent_id] == 0:  # Small penalty for staying still
                reward -= 1
