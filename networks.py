...
class CriticNetwork(nn.Module):
    def __init__(self, beta, n_agents, n_actions, fc1_dims, fc2_dims, name, chkpt_dir):
        ...
        self.fc_layers = nn.Sequential(
            nn.Linear(conv_output_size + n_agents * n_actions, fc1_dims),
            nn.ReLU(),
    ...
    def forward(self, state, action):
        conv_out1 = self.conv_layers(state)
        conv_out2 = conv_out1.view(conv_out1.size(0), -1)  # Flatten the output

        action = action.view(action.size(0), -1)
        # Concatenate the flattened features with the actions
        combined = T.cat([conv_out2, action], dim=1)

        q = self.fc_layers(combined)
...
class ActorNetwork(nn.Module):
    ...
    def forward(self, state):
        conv_out1 = self.conv_layers(state)
        conv_out1 = conv_out1.view(conv_out1.size(0), -1)  # Flatten the output for the fully connected layers
        conv_out2 = self.fc_layers(conv_out1)
        return conv_out2
